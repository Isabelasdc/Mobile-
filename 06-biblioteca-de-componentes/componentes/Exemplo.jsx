import { StatusBar } from 'expo-status-bar';
import { StyleSheet, Text, View } from 'react-native';
import { PaperProvider, Card, Title, Paragraph, Divider, Button} from 'react-native-paper';

export default function App() {

      const lista = ["uva", "Maça", "Banana", "Laranja"]
  
  return (
    <PaperProvider>
    <View style={styles.container}>
      <StatusBar style="auto" />

      <Button mode='contained' onPress={() => alert("Clicou")} >Clique Aqui!</Button>
      <Button icon="camera" mode='contained-tonal'>CLique Aqui!</Button>
      <Button mode='elevated'>CLique Aqui!</Button>
      <Button mode='text'>CLique Aqui!</Button>


      <Text>Uva</Text>
      <Divider />
      <Text>maça</Text>
      <Divider />
      <Text>Banana</Text>
      <Divider />
      <Text>Laranja</Text>
      <Divider />


      <Text variant='titleLarge'>Um texto aí</Text>
      <Text variant='headLineLarge'>Um texto aí</Text>
      <Text variant='displayLarge'>Um texto aí</Text>

      <Card>
        <Card.Content>
          <Title>Título do Card</Title>
          <Paragraph>Um paragrafo qualquer ai, Um paragrafo qualquer ai
          Um paragrafo qualquer ai, Um paragrafo qualquer ai
          , Um paragrafo qualquer ai, Um paragrafo qualquer ai
           </Paragraph>
        </Card.Content>
        <Card.Cover source={{uri: 'https://i.pinimg.com/564x/32/b6/16/32b6163218ac52ce74356102576481e3.jpg'}} />
      </Card>
    </View>
    </PaperProvider>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#fff',
    alignItems: 'center',
    justifyContent: 'center',
  },
});
